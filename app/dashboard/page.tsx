...
export default async function DashboardPage({searchParams}:Props) {
  ...
  const cards = [{ label: "Cobranças abertas", count: charges, href: "/cobrancas", tone: "bg-[#f4ddd2] text-[#804331]" }, { label: "Repasses pendentes", count: transfers, href: "/essa-semana", tone: "bg-[#eee0c9] text-[#71542c]" }, { label: "Contratos vencendo", count: expiring.length, href: "/contratos", tone: "bg-[#e7e0d9] text-[#5e5148]" }];
  ...
  return <><PageHeader eyebrow="Visão geral" title="Dashboard" description="Alertas importantes sem executar nenhuma ação financeira." />
    ...
    <Link href="/essa-semana" className="mt-6 inline-flex min-h-12 items-center justify-center rounded-2xl bg-[var(--brand)] px-5 text-sm font-bold text-white shadow-[0_8px_18px_rgba(91,52,38,0.2)]">Ir para Essa Semana</Link>
  </>;
}
